...
Every talking-head shot from the source is replaced by a neutral placeholder
avatar; all graphic scenes (title cards, diagrams, terminals, docs, charts,
install cards) are redrawn programmatically. Caption timing is driven by
captions.json, which was recovered from the source frames.
...
def pop(x):
    """Spring pop for caption entrances: quick scale with slight overshoot."""
    x = max(0.0, min(1.0, x))
    return 1 + 0.14 * math.sin(x * math.pi) if x < 1 else 1.0
...
def scaled_text(img, xy, s, f, fill, scale, stroke=0, stroke_fill=None):
    """Draw text centered at xy with a scale factor (for pop-in animation)."""
    if scale <= 0:
        return
...
def textured_bg(seed=7, tint=TEX_BG):
    """Bluish paper texture with a soft leaf-like shadow, like the source."""
    key = (seed, tint)
    ...
    # soft diagonal shadow blobs (palm-leaf shadow feel)
...
def robot(img, cx, cy, cell, color=CLAY):
    """Pixel crab mascot. (cx, cy) is the center of the head block."""
    d = ImageDraw.Draw(img)
    rows = len(ROBOT_GRID)
    cols = len(ROBOT_GRID[0])
    x0 = cx - cols * cell // 2
    y0 = cy - rows * cell // 2
    for r, row in enumerate(ROBOT_GRID):
        for c, ch in enumerate(row):
            ...
            elif ch == 'A':   # left arm sticks out
                d.rectangle([box[0] - cell // 2, box[1], box[2], box[3]], fill=color)
            elif ch == 'a':   # right arm sticks out
...
def mac_window(img, box, dark=True, title='', radius=14):
    """macOS-style window chrome; returns the content box below the title bar."""
    d = ImageDraw.Draw(img)
    x0, y0, x1, y1 = box
    bar_h = 34
...
def avatar_placeholder(box, zoom=1.0, corner=0, shift_y=0):
    """Neutral placeholder for every talking-head shot: soft gradient panel
    with a person silhouette. zoom>1 pushes in like the source close-ups."""
    ...
    # soft window glow on the right, like the source backdrop
    glow = Image.new('L', (bw, bh), 0)
    gd = ImageDraw.Draw(glow)
    gd.rounded_rectangle([int(bw * 0.72), int(bh * 0.18), int(bw * 1.05), int(bh * 0.62)],
                         radius=30, fill=34)
    ...
    # silhouette
    cx = bw // 2
    hr = int(min(bw, bh) * 0.30 * zoom)              # head radius
    hy = int(bh * 0.42) + shift_y                    # head center y
    d.ellipse([cx - hr, hy - hr, cx + hr, hy + hr], fill=AV_FIG)
    sw = int(hr * 2.9)                               # shoulders
    sy = hy + int(hr * 1.25)
    ...
    lab.text((cx, min(bh - 24, sy + int(hr * 1.4))), 'AVATAR PLACEHOLDER',
...
def paste_avatar(img, box, zoom=1.0, corner=0, shift_y=0, corners='top'):
    av, mask = avatar_placeholder(box, zoom, corner, shift_y)
    if mask is not None and corners == 'top':
        # square off the bottom corners (card runs off-screen)
...
def doc_icon(img, cx, cy, w, h, tilt=0, label='', watermark=''):
    """White page icon with folded corner, optional tilt and label."""
    ...
    if watermark:
        pd.text((20 + w // 2, 20 + h // 2), watermark,
                font=sans_b(int(w * 0.22)), fill=(225, 222, 216), anchor='mm')
    if tilt:
...
def folder_icon(img, cx, cy, w, h, label=''):
    d = ImageDraw.Draw(img)
    tab_w = int(w * 0.42)
...
# ---------------------------------------------------------------- scenes
# Boundaries measured from the source cut list.
...
def s01_title(img, t, lt):
    """Cream title card: serif headline, small mascot, avatar card below."""
    base = cream_bg()
    img.paste(base, (0, 0))
    d = ImageDraw.Draw(img)
    ...
    robot(img, 160, 470, 18)
    paste_avatar(img, (30, 800, 690, H), zoom=1.05, corner=85, shift_y=-30)


def s02_diagram(img, t, lt):
    """Files fanning into a skill: arrows, pdf, SKILL.md, folder, zip."""
    ...
    for i, (tx, ty) in enumerate(targets):
        sx = 215 + i * 72
        sy = 300 + abs(i - 2) * 6
        d.line([(sx, sy), (tx, ty)], fill=(25, 22, 18), width=3)
        ang = math.atan2(ty - sy, tx - sx)
        for da in (-0.5, 0.5):
            d.line([(tx, ty), (tx - 14 * math.cos(ang + da), ty - 14 * math.sin(ang + da))],
                   fill=(25, 22, 18), width=3)
    if lt > 1.45:  # orange handwritten numbers appear
        nf = serif_bi(46)
        for i, (nx, ny) in enumerate([(255, 220), (310, 205), (365, 195), (420, 205), (478, 222)]):
            if lt > 1.45 + i * 0.04:
                text_mm(d, (nx, ny), str(i + 1), nf, CLAY)
    img.paste(rgba.convert('RGB'), (0, 0))
    rgba = img.convert('RGBA')
    # pdf (tilted, brand deck)
    doc_icon(rgba, 205, 500, 130, 165, tilt=12, label='brand-guidelines.pdf')
    pd = ImageDraw.Draw(rgba)
    ...
    # SKILL.md center + folder
    doc_icon(rgba, 370, 480, 115, 150, label='SKILL.md', watermark='SKILL')
    folder_icon(rgba, 540, 520, 130, 95, label='Resources')
    # zip below
    doc_icon(rgba, 370, 690, 100, 130)
...
def s03_head1(img, t, lt):
    paste_avatar(img, (0, 0, W, H), zoom=1.15)


def s04_skillmd(img, t, lt):
    """Dark SKILL.md document, slowly scrolling up."""
    ...
    rr(d, [78, 256, 222, 294], 6, fill=CLAY)
    d.text((150, 274), 'Find Skills', font=sans_b(22), fill=WHITE, anchor='mm')
    body = (120, 122, 130)
    txt = (208, 210, 216)
    ...
    bullets = [
        ('Asks "how do I do X" where X might be a common', True),
        ('task with an existing skill', True),
        ('Says "find a skill for X" or "is there a skill for X"', False),
        ('Asks "can you do X" where X is a specialized capability', False),
        ('Expresses interest in extending agent capabilities', False),
        ('Wants to search for tools, templates, or workflows', False),
        ('Mentions they wish they had help with a specific', False),
        ('domain (design, testing, deployment, etc.)', False),
    ]
    y = 505
    first = True
    for line, hl in bullets:
        if hl:
            ...
            rr(d, [112, y - 4, 118 + tw + 8, y + 22], 3, fill=CLAY)
        if first or not bullets[max(0, bullets.index((line, hl)) - 1)][0].endswith(('common', 'specific')):
    ...
    for line in ['The Skills CLI ( npx skills ) is the package manager for',
        ...
        y += 27
    off = int(ease(lt / 1.4) * 55)
    img.paste(doc.crop((0, off, W, off + H)), (0, 0))


def s05_terminal(img, t, lt):
    """Textured backdrop, dark terminal typing `ls`, avatar card below."""
    ...
    # toast inside the terminal
    rr(d, [175, 675 - 200, 545, 715 - 200], 6, fill=(45, 45, 45))
    robot(img, 197, 493, 3)
    d = ImageDraw.Draw(img)
    d.text((215, 486), "If you're using Windows, this is in C:/Users/<you>",
           font=mono_r(10), fill=(210, 210, 210))
    paste_avatar(img, (0, 650, W, H), zoom=0.95, corner=100, shift_y=-15)


def s06_install(img, t, lt):
    """Install card with mascot and animated progress bar."""
    img.paste(cream_bg(), (0, 0))
    d = ImageDraw.Draw(img)
    robot(img, 360, 315, 24)
    d = ImageDraw.Draw(img)
...
def s07_head2(img, t, lt):
    paste_avatar(img, (0, 0, W, H), zoom=1.1)


def s08_terminal2(img, t, lt):
    """Claude Code terminal card sliding in; research prompt being typed."""
    img.paste(textured_bg(seed=11), (0, 0))
    slide = int((1 - ease(min(1, lt / 0.6))) * -160)
    top = 160 + slide
    box = (75, top, 645, top + 325)
    d = ImageDraw.Draw(img)
    rr(d, [box[0] + 6, box[1] + 10, box[2] + 6, box[3] + 10], 14, fill=(178, 172, 196))
    cb = mac_window(img, box, dark=True, title='Claude Code')
    d = ImageDraw.Draw(img)
    # striped mascot header strip
    for i in range(4):
    ...
    d.text((box[0] + 40, yb), ln1, font=mono_r(15), fill=(230, 230, 230))
    if ln2:
        d.text((box[0] + 40, yb + 20), ln2, font=mono_r(15), fill=(230, 230, 230))
    if lt > 2.2:
        d.text((box[0] + 20, yb - 44), 'Considering...', font=mono_r(15), fill=(200, 160, 90))
...
def s09_plan(img, t, lt):
    """White card: PROPERLY / ascii-wave window with surfing mascot / PLAN."""
    ...
    for _ in range(2600):
        ...
        r = abs(rnd.gauss(0, 1))
        # wave-crest shape: lobe up-left plus a long base
        if rnd.random() < 0.55:
            px = cx0 - 60 + math.cos(a) * r * 150 * 0.9
        ...
        d.text((px, py), rnd.choice(chars), font=mono_r(rnd.choice([8, 9, 10, 11])),
               fill=(g, g, g))
    # blue wave board + mascot riding it
    for _ in range(240):
        px = cx0 - 130 + rnd.uniform(0, 190)
        py = cy0 + 18 + rnd.uniform(0, 26)
        d.rectangle([px, py, px + 7, py + 7], fill=(126, 168, 222))
    for _ in range(40):
        px = cx0 - 40 + rnd.uniform(0, 70)
        py = cy0 + 14 + rnd.uniform(0, 16)
        d.rectangle([px, py, px + 6, py + 6], fill=(214, 108, 166))
    robot(img, 330, 548, 13)
    if lt > 0.55:
        d = ImageDraw.Draw(img)
        text_mm(d, (360, 1052), 'PLAN', serif_bi(84), (14, 12, 10))


def s10_code(img, t, lt):
    """Dark scene: Claude Code session window over avatar lower half."""
    ...
    scroll = int(ease(lt / 2.8) * 120)
    img.paste(win.crop((0, scroll, 520, scroll + 420)), (100, 140))
    paste_avatar(img, (0, 660, W, H), zoom=0.95, shift_y=-20)
...
def s12_claudemem(img, t, lt):
    """claude-mem GitHub README page on near-black."""
    ...
    ly = ty + 150
    for lx, wds in [(70, 70), (150, 84), (244, 82), (336, 96), (60, 120), (190, 60)]:
        yy = ly if lx > 60 or wds != 120 else ly + 18
    ...
    off = int(ease(lt / 1.7) * 60)
    img.paste(page.crop((0, off, 520, off + 660)), (100, 215))


def s13_starhistory(img, t, lt):
    """White scene: Star History chart line growing; avatar card below."""
    ...
    ax0, ay0, ax1, ay1 = 95, 235, 665, 515   # plot area
    ...
    # growth curve reveals over the scene
    frac = ease(min(1.0, lt / 2.9))
    pts = []
    ...
    star = (98, 168, 74)
    d.text((565, 540), '* star-history.com', font=sans_r(13), fill=star)
    paste_avatar(img, (0, 660, W, H), zoom=0.95, corner=100, shift_y=-15)


def s14_head4(img, t, lt):
    paste_avatar(img, (0, 0, W, H), zoom=1.12)


def s15_impeccable(img, t, lt):
    """Black card: Start with /impeccable + three numbered steps."""
    ...
    y = 378
    for num, tag, cmd, sub in steps:
        d.text((95, y + 6), num, font=mono_r(13), fill=(224, 176, 66))
        d.text((131, y), tag, font=mono_r(12), fill=(150, 150, 150))
        d.text((131, y + 18), cmd, font=mono_b(15), fill=(235, 235, 235))
        d.text((131, y + 40), sub, font=sans_r(13), fill=(170, 170, 170))
        y += 90
    d.line([(85, y - 12), (635, y - 12)], fill=(40, 40, 40))


def s16_premium(img, t, lt):
    """White scene, dark rounded card with 2x3 grid of site critiques."""
    img.paste(Image.new('RGB', (W, H), (250, 250, 249)), (0, 0))
    card = Image.new('RGB', (520, 780), (10, 10, 10))
    ...
    for i, (kind, label, lines) in enumerate(cells):
        ...
        if label:
            d.text((cx0 + 12, cy0 + 162), label, font=sans_b(15), fill=(235, 200, 90) if False else (240, 240, 240))
            yy = cy0 + 186
            for ln in lines:
                d.text((cx0 + 12, yy), ln, font=sans_r(11), fill=(175, 175, 175))
                yy += 16
    scroll = int(ease(lt / 1.7) * 120)
...
def s17_slop(img, t, lt):
    """Black scene: vibe-coded landing page mock with a red slash."""
    img.paste(Image.new('RGB', (W, H), (3, 3, 3)), (0, 0))
    ...
    for j, (seg, col) in enumerate([('Build faster with ', (235, 235, 245)),
                                    ('intelligent', (196, 130, 240)),
                                    ]):
        pass
...
def s19_taskobserver(img, t, lt):
    """task-observer GitHub README on dark."""
    ...
    for ln in ['1.  Creates new skills for you - it spots repeating patterns in your',
        ...
        y += 20
    off = int(ease(lt / 1.4) * 40)
    img.paste(page.crop((0, off, 560, off + 720)), (80, 180))


def s20_taskcard(img, t, lt):
    """Cream card: mascot observing + Claude Code checklist window."""
    img.paste(textured_bg(seed=15), (0, 0))
    d = ImageDraw.Draw(img)
    rr(d, [70, 200, 650, 520], 26, fill=CREAM_CARD)
    rr(d, [70, 200, 650, 520], 26, outline=(212, 206, 196), width=1)
    hop = int(abs(math.sin(lt * 2.4)) * 6)
    robot(img, 195, 345 - hop, 14)
    d = ImageDraw.Draw(img)
    d.ellipse([160, 398, 230, 408], fill=(214, 208, 198))
    rr(d, [150, 418, 246, 440], 10, fill=(250, 247, 243))
    d.ellipse([158, 426, 165, 433], fill=(216, 90, 60))
    d.text((171, 422), 'Observing tasks...', font=sans_r(11), fill=(90, 86, 80))
    # mini Claude Code window
    rr(d, [318, 250, 600, 472], 10, fill=(251, 247, 244))
    rr(d, [318, 250, 600, 472], 10, outline=(220, 214, 204), width=1)
    for i, c in enumerate([(255, 95, 86), (255, 189, 46), (39, 201, 63)]):
        d.ellipse([328 + i * 10, 258, 334 + i * 10, 264], fill=c)
    d.text((370, 256), 'Claude Code', font=sans_b(11), fill=(70, 66, 60))
    ndone = 1 if lt < 2.2 else 2
    d.text((588, 256), f'{ndone + 1}/6', font=sans_r(10), fill=(140, 136, 130), anchor='ra')
    ...
    for i, (rowt, meta) in enumerate(zip(rows, metas)):
        ...
        mcol = (216, 90, 60) if meta == 'running' and active else (170, 166, 160)
        d.text((588, y - 1), meta if (i == ndone and meta == 'running') or i != ndone else 'running',
               font=mono_r(9) if meta == 'running' else sans_r(10), fill=mcol, anchor='ra')
        y += 29
    paste_avatar(img, (0, 650, W, H), zoom=0.95, corner=100, shift_y=-15)


def s21_outro(img, t, lt):
    paste_avatar(img, (0, 0, W, H), zoom=1.3, shift_y=40)
